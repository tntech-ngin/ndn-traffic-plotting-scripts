...
class PacketsHistogramThroughput:
    ...
    async def plot(self, duration):
        ...
        packet_data = [(i_ts, i_sizes, interest_num_packets, interest_ts, interest_throughput),
        ...
        for packets, sizes, num_packets, ts, throughput in packet_data:
            for packet_t, size in zip(packets, sizes):
                ...
                if num_packets is not None and ts is not None:
                    ...
                    ts[packet_duration] = datetime.utcfromtimestamp(
                        packet_t / 1e9)
        throughput = [t * 8 / (duration * 60) / 1e6 for t in (
            interest_throughput[i] + data_throughput[i] + r_throughput[i] for i in range(num_durations))]
        ...
        fig, (ax1, ax2) = plt.subplots(nrows=2, ncols=1,
                                       figsize=(8, 8))
        # First plot
        ax1.plot(np.arange(num_durations), [i / 1000 for i in interest_num_packets],
                 color='#139061', label='Interests')
        ax1.plot(np.arange(num_durations),
                 [i / 1000 for i in data_num_packets], color='#AC9820', label='Data')

        xformatter = ticker.FuncFormatter(lambda x, pos: (
            start_time + timedelta(minutes=x * duration)).strftime('%H:%M'))
